...
import { Staff, generateStaffData } from "@/data/dummy";
...
const TableStaff = () => {
  ...
  const [tempData, setTempData] = React.useState<Staff[]>([]);
  ...
  const filterData = [
    ...
    },
    {
      ...
      onSelectFilter: (value: string) => {
        if (value === "") return setData(tempData);
        const filteredData = tempData.filter((item) => item.shift === value);
        setData(filteredData);
      },
    },
  ];
  ...
  React.useEffect(() => {
    const users = generateStaffData(100);
    setData(users);
    setTempData(users);
    setIsLoaded(true);
  }, []);
